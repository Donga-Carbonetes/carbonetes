    ...
    while True:
        for ep in NODE_EXPORTERS:
            try:
                ...
                prev_cpu = last_snapshots.get(ep)
                ...
                if prev_cpu:
                    ...
                    report_metrics(ep, cpu_usage, mem_usage)
                else:
                    logger.info(f"[{ep}] 첫 스냅샷 저장 (CPU): {curr_cpu} | MEM total={total}, avail={avail}")

                last_snapshots[ep] = curr_cpu
