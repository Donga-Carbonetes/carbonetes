...
def learning_loop():
    ...
    # [3] 초기 개체군 형성
    candidates_weight = generate_candidates(
        n=5, include_current=True)  # n 가중치 후보 갯수

    # [4] 시뮬레이션(재실행 가상화)
    sim_result = run_simulation_as_dicts_from_modules(
        task_data, candidates_weight)
    ...
    # [6] 지역 탐색 수행
    best_result = local_search_with_sim(
        best_result, task_data, alpha=1.0, gamma=0.05, epsilon=0.02)
...
def save_best_weights_to_db(best_result: dict):
    ...
    try:
        ...
        values = (best_weights['a'], best_weights['b'],
                  best_weights['c'], best_weights['d'])
